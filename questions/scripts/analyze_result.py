...
def commandLine():
    ...
    parser.add_argument("-f", "--file", help="Input path to CSV file/files", required=True, nargs="*")
    parser.add_argument("-o", "--out", help="Input path to the output folder", required=True)
    ...
    parser.add_argument("-cu", "--custom", help="Input path to the text file containing custom counts for the category 'Other'")
    parser.add_argument("-thres", "--threshold", help="Save all rows (with Question ID, Artifact, Phrase, P(i) value and category name) with a P(i) value higher then the threshold entered in an extra XML file", type=float)
    ...
    if(custom != None):
        customCheck = True
        with open(custom, "r") as creader:
            content = creader.readlines()
            for line in content:

                if(line):
                    line_split = line.split("=")
                    customDic[line_split[0]] = line_split[1].split(",")
...
def parseResults(surveys):
    ...
    top = "Question ID,Artifact,P(i),Sum over all n(ij),Total Responses,"
    ...
    for question in survey:
        ...
        if(currentColumn >= column):
            ...
            for p in points:
                ...
                if(points[p] > highest_category):
                    highest_category = points[p]
                    highest_category_name = p

            rowResults = rowResults + "," + others[:-2].replace(",",";") + "," + comments[:-2].replace(",",";")
            fileResults = fileResults + localTitle + "," + noun + rowResults + "\n"

            if(threshold != None and p_i > threshold):
                id = titleID.split("[")[1].split("]")[0]
                if(not title in threshold_p_i_dict.keys()):
                    threshold_p_i_dict[title] = {}

                threshold_p_i_dict[title][id] = {}
                threshold_p_i_dict[title][id]["question"] = question.split(".")[1].split("[")[0]
                threshold_p_i_dict[title][id]["noun"] = noun
                threshold_p_i_dict[title][id]["category"] = category
                threshold_p_i_dict[title][id]["agreement"] = str(p_i)
    ...
    with open(output.replace("//", "/") + "lsg__" + resultName + ".csv", "w") as lsg:
        lsg.write(top + fileResults + summary)

    if(threshold != None):
        threshold_str = "<BIODIV-QUESTIONS>\n"
        for key in threshold_p_i_dict.keys():
            firstKey = next(iter(threshold_p_i_dict[key]))
            threshold_str = threshold_str + "<BIODIV-QUESTION questionid=\"" + key + "\">\n\t<QUESTION>" + threshold_p_i_dict[key][firstKey]["question"] + "</QUESTION>\n\t<KEYWORDS></KEYWORDS>\n\t<ARTIFACTS>"
            for id in threshold_p_i_dict[key].keys():

                threshold_str = threshold_str + "\n\t\t<ARTIFACT subqid=\"" + id + "\">\n\t\t\t<PHRASE>" + threshold_p_i_dict[key][id]["noun"] + "</PHRASE>\n"
                threshold_str = threshold_str + "\t\t\t<AGREEMENT>" + threshold_p_i_dict[key][id]["agreement"] + "</AGREEMENT>\n"
                threshold_str = threshold_str + "\t\t\t<CATEGORY>" + threshold_p_i_dict[key][id]["category"] + "</CATEGORY>\n\t\t</ARTIFACT>"

            threshold_str = threshold_str + "\n\t</ARTIFACTS>\n</BIODIV-QUESTION>\n"

        threshold_str = threshold_str + "</BIODIV-QUESTIONS>"

        with open(output.replace("//", "/") + "threshold_lsg__" + resultName + ".xml", "w") as threshold_lsg:
            threshold_lsg.write(threshold_str)
