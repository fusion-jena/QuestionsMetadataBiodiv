 ...
 * ```-d``` deletes the 'selected_files' folder if it already exists in the directory. If the option wasn't set but the 'selected_files' folder exists, an error is thrown.
 ...
 ```get_topN.py``` is an easy to use Command Line Interface (CLI) tool to get the top N number of keywords in the 'keyword_counts' CSV-file produced by the ```-f``` option of the ```split_content.py``` script. This can be helpful if you want, for example, only the top 50 used keywords but the 'keyword_counts' CSV-file is very large and can't be opened with an ordinary CSV program (i.e. Excel or LibreOffice). The results will be saved in a CSV-file with the added string 'getTopN' in the same directory as the 'keyword_counts' CSV-file. The script has two options as input, ```-c``` and ```-n```.
